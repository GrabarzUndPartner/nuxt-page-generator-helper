...
export async function cacheRoutes (moduleScope, options) {
  ...
  cacheEmpty = !options.routesCache || (options.isDev && options.routesCache && !routes) || 'npm_config_virtual_content_clear_cache' in process.env
...
export function createRoutesCache (nuxt, routes) {
  const filepath = getLocaleCachePath(nuxt)
...
export function getLocaleCachePath (nuxt) {
...
export function cleanCacheDir (nuxt) {
  return new Promise((resolve) => {
    rimraf(getCachePath(nuxt), {}, resolve)
  })
}
